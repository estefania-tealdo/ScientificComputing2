...
class Cell(object, metaclass=ABCMeta):
    ...
    def create_neighbor_cell_index(self, neighbor_delta_coordinate, domain=1):
        ...
        self.neighbor_cell_index = []
        ############## Task 1.2 begins ##################
        #relativeCoordsLst = get_successor_neighbor_delta_coordinate(a)
        lx = self.cell_center[0] - self.side_length/2
        ly = self.cell_center[1] - self.side_length/2
        for coordinates in neighbor_delta_coordinate:
            #self.cell_center = [X, Y], coordinates = relative[dX, dY]
            #neighbor_cell_index = [X +- dX, Y +- dY]
            if(self.side_length*(lx + coordinates[0]) < domain and self.side_length*(ly + coordinates[1]) < domain):
                self.neighbor_cell_index.append( [lx + coordinates[0], ly + coordinate[1]] )
            if( self.side_length*(lx - coordinates[0]) > 0 and self.side_length*(ly + coordinate[1]) < domain ):
                self.neighbor_cell_index.append( [lx - coordinates[0], ly + coordinate[1]] )
            if( self.side_length*(lx + coordinates[0]) < domain and self.side_length*(ly - coordinate[1]) > 0 ):
                self.neighbor_cell_index.append( [lx + coordinates[0], ly - coordinate[1]] )
            if( self.side_length*(lx - coordinates[0]) > 0 and self.side_length*(ly - coordinate[1]) > 0 ):
                self.neighbor_cell_index.append( [lx - coordinates[0], ly - coordinate[1]] )
        # try [self.lx +- i, self.ly +-j]
        ############## Task 1.2 ends ##################
